...
export default function CheckoutPage() {
  ...
  // Clear cart on confirmation step (only once) and store purchased items
  useEffect(() => {
    if (currentStep === 2 && !cartCleared) {
      setPurchasedItems(cartItems);
      clearCart();
      setCartCleared(true);
      if (cartItems.length > 0) {
        // Add purchased services to user state
        if (typeof window !== "undefined") {
          // Avoid hydration mismatch
          import("@/store/auth-store").then(({ useAuthStore }) => {
            useAuthStore.getState().addPurchasedServices(cartItems);
          });
        }
      }
    }
  }, [currentStep, cartCleared, clearCart, cartItems]);
  ...
  // Use the same service fee and taxes as the cart page
  const serviceFee = 15;
  const taxes = 10;
          ...
          {currentStep === 0 && (
            <>
              ...
              <div className="w-full flex justify-center">
                <Button
                  className="w-full sm:w-2/3 text-lg"
                  onClick={handleNext}
                >
                  Continue to Payment
                </Button>
            ...
            <>
              <div className="text-center mb-6" ref={confirmationRef}>
                ...
                <div className="bg-muted/50 rounded-lg p-4 mb-6 text-left max-w-md mx-auto">
                  ...
                  {(purchasedItems.length > 0 ? purchasedItems : cartItems).map(
                    (item) => (
