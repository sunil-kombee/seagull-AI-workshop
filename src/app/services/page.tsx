...
export default function ServicesPage({ searchParams }: ServicesPageProps) {
  ...
  if (selectedCategory !== "all") {
    const categoryObject = serviceCategories.find(cat => cat.slug === selectedCategory);
    if (categoryObject) {
      filteredServices = filteredServices.filter(service => service.category === categoryObject.name);
    }
  }
    ...
    <MainLayout>
      ...
      {filteredServices.length > 0 ? (
        ...
        <div className="text-center py-16">
          <h2 className="text-2xl font-semibold font-headline text-muted-foreground">No services found</h2>
          <p className="mt-2 text-foreground">Try adjusting your search or filters.</p>
        </div>
      )}
    </MainLayout>
  );
